...
def process_request(request_id, api_key, style, topic):
    ...
    task = f"Give me a viral youtube shorts (less than 60 seconds!) script about {topic}. The background music of the video will be {style}. Separate the script into scenes. Each scene is separated by a ';'. Only output the script text. Don't output any meta descriptions like 'scene:' or (opening shot) etc., just the text. Make sure the topic is trending and edited."
